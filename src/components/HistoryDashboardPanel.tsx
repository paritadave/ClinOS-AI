          ...
          >
            ...
            {activeTab === "trends" && (
              <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
                {/* Left Side: Graph */}
                <div className="md:col-span-8 bg-slate-50/50 border border-slate-200 rounded-2xl p-5 min-h-[380px] flex flex-col justify-between">
                  <LongitudinalTrendGraph patient={patient} onLogAudit={onLogAudit} />
                </div>

                {/* Right Side: Lab limits and Meds stack */}
            ...
            {activeTab === "integrations" && (
              <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
                {/* Left Column: Gateways, Sync trigger, and Patient Intake portal dispatch */}
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-3">
                    ...
                    <div className="space-y-2">
                      {integrations.map(int => (
                        <div key={int.id} className="bg-white p-2.5 rounded-xl border border-slate-150 flex items-center justify-between text-[10px]">
                          <div className="space-y-0.5">
                            <span className="font-bold text-slate-700">{int.provider} ({int.province})</span>
                            <p className="text-slate-400 text-[9px] truncate max-w-[150px]">{int.endpointUrl}</p>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className={`w-1.5 h-1.5 rounded-full ${int.status === "Connected" ? "bg-emerald-500" : "bg-slate-300"}`} />
                            <span className="font-bold text-slate-500">{int.status}</span>
                    ...
                    <div className="pt-2">
                      <button
                        ...
                        onClick={handleEMRSync}
                        disabled={isSyncing}
                        className="w-full bg-[#0F172A] hover:bg-slate-800 text-white font-bold py-2 rounded-xl text-xs flex items-center justify-center gap-1.5 cursor-pointer shadow-xs transition-colors"
                      >
                        <RefreshCw className={`w-3.5 h-3.5 ${isSyncing ? "animate-spin" : ""}`} />
                        {isSyncing ? "Compiling FHIR Resource..." : "Execute Real-time EMR Sync"}
                      </button>
                ...
                {/* Right Column: Payload & Compliance */}
                <div className="md:col-span-7 flex flex-col gap-5">
                  {/* Raw payload */}
                  <div className="bg-slate-900 text-slate-100 p-5 rounded-2xl font-mono text-xs flex-1 flex flex-col justify-between">
                    <div>
                      <span className="text-[10px] text-indigo-400 block mb-2 font-bold uppercase flex items-center gap-1.5">
                        <FileJson className="w-4 h-4" /> HL7 FHIR Sandbox Payload Stream
                      </span>
                      <p className="text-[9px] text-slate-400 mb-3 font-sans">Raw transaction payload generated for provincial EMR gateway sync</p>
                    </div>
                    <pre className="text-[10px] leading-normal bg-slate-950 p-3 rounded-lg overflow-x-auto h-[160px] text-indigo-300">
                  ...
                  <div className="bg-slate-950 text-slate-300 p-5 rounded-2xl font-mono text-xs flex-1 space-y-3">
                    ...
                    <div className="space-y-2 text-[10px] bg-slate-900 p-3 rounded-xl border border-slate-800/80">
                      ...
                      <div className="flex items-center justify-between">
                        <span>HL7 FHIR REST Version Support</span>
                        <span className="text-slate-400 font-bold">v4.0.1 (R4)</span>
                      </div>
                    </div>

                    <div className="text-[9px] text-slate-500 italic font-sans leading-relaxed">
                      *All communication links are encrypted with TLS 1.3 and stored within Canadian Sovereign boundaries.
                    </div>
                  </div>
                </div>
              </div>
